...
const CalendarWeek = ({ events }) => {
  ...
  const getWeekDates = () => {
    const startOfWeek = currentDate.getDate() - currentDate.getDay(); // Start from Sunday
    let dates = [];
    for (let i = 0; i < 7; i++) {
  ...
  return (
    <div className="w-full h-full grid grid-cols-12 gap-5 p-4">
      ...
      <div className="col-span-9 grid grid-cols-7 gap-5">
        {weekDates.map((day, index) => (
          <div
            key={index}
            className={`border p-4 cursor-pointer ${selectedDay === day ? 'bg-blue-200' : ''}`}
          ...
          >
            ...
            <div className="flex justify-center items-center space-x-1 mt-2">
              {groupedEvents[day.fullDate] && groupedEvents[day.fullDate].length > 0 ? (
                groupedEvents[day.fullDate].map((event, index) => (
                  <span key={index} className="w-2 h-2 bg-red-500 rounded-full"></span>
                ))
              ) : null}
            </div>
          </div>
